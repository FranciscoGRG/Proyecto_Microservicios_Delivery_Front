...
export class AuthService {
  ...
  private loggedInUser: WritableSignal<CurrentUserData | null> = signal(this.getInitialUser());

  constructor() {}

  private getInitialUser(): CurrentUserData | null {
    const userJson = localStorage.getItem(this.USER_DATA_KEY);
    if (userJson) {
      try {
        return JSON.parse(userJson) as CurrentUserData;
      } catch (error) {
        this.logout();
        return null;
      }
